...
class AcceptedTap extends StatefulWidget {
  const AcceptedTap({Key? key}) : super(key: key);

  @override
  State<AcceptedTap> createState() => _AcceptedTapState();
}

class _AcceptedTapState extends State<AcceptedTap> {
  late final cubit = InvitationsCubit.get(context);
  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    cubit.endPoint = 'accept';
            ...
            builder: (context, state) {
              ...
              return ListView(
                children: List.generate(cubit.invitationsModel!.data.length, (index) {
                  return Column(
                    children: [
                      InvitationCard(event: cubit.invitationsModel!.data[index], wait: false,isActive: true,),
